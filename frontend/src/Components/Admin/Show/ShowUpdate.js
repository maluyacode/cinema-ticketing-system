...
const ShowUpdate = () => {
    ...
    const updateShow = async (showData) => {
        ...
        try {
            const { data: { show, success } } = await axios.put(`${process.env.REACT_APP_API}/api/v1/show/update/${id}`, showData, config);
            if (success) {
                setLoadingButton(false)
                Toast.success('Show updated', 'top-right')
    ...
    const cinemaWithActiveShows = async (cinema_id) => {
        if (!id) {
            setSingleCinema(null)
            setActiveShows(null)
            formik.setFieldValue('num_of_tickets', 0);
            setNumOfTickets(0)
            return;
        }
        try {
            const { data: { cinema, shows } } = await axios.get(`${process.env.REACT_APP_API}/api/v1/cinema/with-active-shows/${cinema_id}?showId=${id}`, config);
    ...
    const getShow = async () => {
        try {
            const { data: { show } } = await axios.get(`${process.env.REACT_APP_API}/api/v1/show/only-one/${id}`, config);
            setShow(show);
            const defaultMovieValue = {
                title: show.movie.title,
                image: show.movie.images[0].url,
                description: show.movie.description,
                id: show.movie._id
            }
            const defaultCinemaValue = {
                name: show.cinema.name,
                location: show.cinema.location,
                image: show.cinema.images[0].url,
                screen_type: show.cinema.screen_type.name,
                description: show.cinema.screen_type.description,
                seat_layout: show.cinema.seat_layout,
                id: show.cinema._id
            }

            setSelectedCinema(defaultCinemaValue)
            setSelectedMovie(defaultMovieValue)

            formik.setFieldValue('movie', show.movie._id)
            formik.setFieldValue('ticket_price', show.ticket_price)
            formik.setFieldValue('cinema', show.cinema._id)
            const initialNumOfTickets = show.cinema.seat_layout.reduce((prev, curr) => prev + curr.column, 0)
            formik.setFieldValue('num_of_tickets', initialNumOfTickets)
            setNumOfTickets(initialNumOfTickets)
            formik.setFieldValue('date_show', dayjs(show.start_time))
            formik.setFieldValue('start_time', dayjs(show.start_time))
            formik.setFieldValue('end_time', dayjs(show.end_time))
            cinemaWithActiveShows(show.cinema._id);
            setImagesPreview(show.images.flatMap(image => image.url));
        ...
        <Box sx={{ display: 'flex' }}>
            ...
            <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
                ...
                <Container columns={13} gap={5} sx={{ display: 'flex', justifyContent: 'center' }}>
                    ...
                    >
                        <Typography variant='h5' p={2}>Update Show</Typography>
                        <Divider light={false} />
