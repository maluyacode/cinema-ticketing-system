...
const UserUpdate = () => {
    ...
    const updateUser = async (userData) => {
        ...
        try {
            const { data: { user, success } } = await axios.put(`${process.env.REACT_APP_API}/api/v1/user/update-by-admin/${id}`, userData, config);
            if (success) {
                setLoadingButton(false)
                Toast.success('User updated', 'top-right')
    ...
    const getUser = async () => {
        try {
            const { data: { user } } = await axios.get(`${process.env.REACT_APP_API}/api/v1/single-user/${id}`, config);
        ...
        <Box sx={{ display: 'flex' }}>
            ...
            <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
                ...
                <Container sx={{ display: 'flex', justifyContent: 'center' }}>
                    ...
                    >
                        <Typography variant='h5' p={2}>Update User</Typography>
                        <Divider light={false} />
                        <Grid container py={2} px={4} my={2} gap={3} columns={13}>
                            ...
                            <Grid item xl={13} md={13} sm={13} xs={13}>
                                <TextField label="New password" variant="outlined" size='small' fullWidth
                                    type='password'
                                    name='password'
